...
class HDDM_W_test(SuperDetector):
    ...
    def run(self, pr):
        ...
        if self.total.EWMA_estimator < 0.0:
            ...
            self.total.EWMA_estimator = (
                self.lambda_ * pr + aux_decay_rate * self.total.EWMA_estimator
            )
        ...
        if self.test_type != "one-sided" and self.monitor_mean_decr(
    ...
    def update_incr_statistics(self, pr):
        ...
        bound = math.sqrt(
            self.total.independent_bounded_condition_sum
            * math.log(1.0 / self.drift_confidence, math.e)
            / 2
        )
        ...
        if self.total.EWMA_estimator + bound < self.incr_cut_point:
            ...
            self.sample1_incr_monitoring.independent_bounded_condition_sum = (
                self.total.independent_bounded_condition_sum
            )
            self.sample2_incr_monitoring = SampleInfo()
        else:
            if self.sample2_incr_monitoring.EWMA_estimator < 0.0:
                ...
                self.sample2_incr_monitoring.EWMA_estimator = (
                    self.lambda_ * pr
                    + aux_decay * self.sample2_incr_monitoring.EWMA_estimator
                )
                self.sample2_incr_monitoring.independent_bounded_condition_sum = (
                    self.lambda_ * self.lambda_
                    + aux_decay
                    * aux_decay
                    * self.sample2_incr_monitoring.independent_bounded_condition_sum
    ...
    def monitor_mean_incr(self, confidence_level):
        return self.detect_mean_increment(
            self.sample1_incr_monitoring, self.sample2_incr_monitoring, confidence_level
        )
    ...
    def detect_mean_increment(sample_1, sample_2, confidence_level):
        ...
        bound = math.sqrt(
            (
                sample_1.independent_bounded_condition_sum
                + sample_2.independent_bounded_condition_sum
            )
            * math.log(1 / confidence_level, math.e)
            / 2
        )
    ...
    def update_decr_statistics(self, pr):
        ...
        if self.total.EWMA_estimator - epsilon > self.decr_cut_point:
            ...
            self.sample1_decr_monitoring.independent_bounded_condition_sum = (
                self.total.independent_bounded_condition_sum
            )
            self.sample2_decr_monitoring = SampleInfo()
        else:
            if self.sample2_decr_monitoring.EWMA_estimator < 0.0:
                ...
                self.sample2_decr_monitoring.EWMA_estimator = (
                    self.lambda_ * pr
                    + aux_decay * self.sample2_decr_monitoring.EWMA_estimator
                )
                self.sample2_decr_monitoring.independent_bounded_condition_sum = (
                    self.lambda_ * self.lambda_
                    + aux_decay
                    * aux_decay
                    * self.sample2_decr_monitoring.independent_bounded_condition_sum
    ...
    def monitor_mean_decr(self, confidence_level):
        return self.detect_mean_increment(
            self.sample2_decr_monitoring, self.sample1_decr_monitoring, confidence_level
        )
    ...
    def get_settings(self):
        return [
            str(self.drift_confidence)
            + "."
            + str(self.warning_confidence)
            + "."
            + str(self.lambda_)
            + "."
            + str(self.test_type),
            "$\delta_d$:"
            + str(self.drift_confidence).upper()
            + ", "
            + "$\delta_w$:"
            + str(self.warning_confidence).upper()
            + ", "
            + "$\lambda$:"
