...
class FHDDMS_add(SuperDetector):
    ...
    def run(self, pr):
        ...
        if self._counter % self._ELEMENT_SIZE == 0:
            ...
            if self._mu_max_short - m_temp > self.__cal_hoeffding_bound(
                self._ELEMENT_SIZE
            ):
                return False, True
        ...
        if self._counter == len(self._stack) * self._ELEMENT_SIZE:
            ...
            if self._mu_max_large - m_temp > self.__cal_hoeffding_bound(
                len(self._stack) * self._ELEMENT_SIZE
            ):
                return False, True
    ...
    def get_settings(self):
        return [
            ...
            + ", "
            + "$n_l$:"
            + str(len(self._stack) * self._ELEMENT_SIZE)
            + ", "
            + "$\delta$:"
