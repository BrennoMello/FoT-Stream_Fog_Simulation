...
class FHDDMS(SuperDetector):
    ...
    def run(self, pr):
        ...
        if len(self._WIN) == self._WIN_SIZE:
            ...
            if self._mu_max_short - sub_wins_mu[
                self._S_WIN_NUM - 1
            ] > self.__cal_hoeffding_bound(self._S_WIN_SIZE):
                return False, True
            ...
            if self._mu_max_large - mu_long > self.__cal_hoeffding_bound(
                self._WIN_SIZE
            ):
                return False, True
    ...
    def get_settings(self):
        return [
            str(self._S_WIN_SIZE) + "." + str(self._WIN_SIZE) + "." + str(self._DELTA),
            "$n_s$:"
            + str(self._S_WIN_SIZE)
            + ", "
            + "$n_l$:"
            + str(self._WIN_SIZE)
            + ", "
            + "$\delta$:"
            + str(self._DELTA).upper(),
        ]
