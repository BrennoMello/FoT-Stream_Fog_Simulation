...
class SeqDrift2ChangeDetector(SuperDetector):
    ...
    def get_settings(self):
        return [
            str(self.DELTA) + "." + str(self.BLOCK_SIZE),
            "$\delta$:"
...
class SeqDrift2:
    ...
    def getDriftType(self):
        if self.getWidth() > self.blockSize:
            ...
            if self.instanceCount > self.blockSize and self.leftReservoir.getSize() > 0:
                if self.epsilon <= abs(
                    self.rightRepositoryMean - self.leftReservoirMean
                ):
                    return self.DRIFT
                else:
                    return self.NODRIFT
    ...
    def getVariance(self):
        ...
        size = self.getWidth()
        x = (
            self.getTotal() * meanminum1 * meanminum1
            + (size - self.getTotal()) * mean * mean
        )
        y = size - 1
        return x / y
    ...
    def optimizeEpsilon(self):
        ...
        if tests >= 1:
            ...
            while IsNotOptimized:
                ...
                if (
                    (previousStepEpsilon - currentStepEpsilon) / previousStepEpsilon
                ) < 0.0001:
    ...
    def adjustForDataRate(self, _dKr):
        meanIncrease = (
            self.rightRepository.getSampleMean() - self.leftReservoir.getSampleMean()
        )
        dk = _dKr
        if meanIncrease > 0:
            dk += (
                (-1) * (meanIncrease * meanIncrease * meanIncrease * meanIncrease) + 1
            ) * _dKr
        elif meanIncrease <= 0:
            dk = _dKr
        return dk
...
class Repository:
    ...
    def get(self, _iIndex):
        return self.blocks[int(_iIndex / self.blockSize)].data[
            (_iIndex % self.blockSize)
        ]

    def addAt(self, _iIndex, _dValue):
        self.blocks[int(_iIndex / self.blockSize)].addAt(
            (_iIndex % self.blockSize), _dValue
        )
...
class Block:
    ...
    def add(self, _dValue):
        if self.indexOfLastValue < len(self.data):
            ...
            print(
                "Error in adding to Block. Last Index:",
                self.indexOfLastValue,
                "Total",
                self.total,
                "Array Length:",
                len(self.data),
